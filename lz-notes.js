...
(function () {
  ...
  var NOTES = [
    ...
    { id: "n-apps-iterationsstufen-offen", page: "sprints", selector: ".bboard tbody tr:nth-child(4)", status: "offen", done: true,
      title: "Iterationsstufen benannt: die neuen Apps sind V3",
      text: "GEFUNDEN 22.08.2026. \"LebensZeit Zuhause\" und \"LebensZeit Team\" tragen im Rail beide \"Prototyp V1\", obwohl LebensZeit OS (/app) der frueher entstandene Stand ist. Damit gibt es drei lauffaehige Staende und zwei davon behaupten dieselbe Stufe. Baustein 4 verlangt drei dokumentierte Iterationsstufen; die Zuordnung ist der Pflichtnachweis und keine Formsache. Nicht selbst entschieden, weil die Reihenfolge eine inhaltliche Festlegung ist — moeglich waere V1 = LebensZeit OS, V2 = Zuhause, V3 = Team, oder eine Trennung nach Zielgruppe statt nach Zeit. ENTSCHIEDEN 22.08.2026 durch Josh: die beiden neuen Staende sind V3. Damit steht die Leiter V1 = LebensZeit OS (/app, SwiftUI-Portierung) · V2 = Kunden-App in Portal-Optik mit den auf Stemmer und Doerre umgestellten Personas · V3 = Zuhause und Team. Brand-Sub in beiden Dateien auf \"Prototyp V3\" geaendert, Baustein-Board benennt alle drei Stufen. Offen bleibt eine Kleinigkeit mit Gewicht: V2 liegt als kunden-app_ALT-portal-optik_2026-08-22.html ausserhalb des Repos und ist nicht aufrufbar. Fuer Baustein 4 ist eine beschriebene Stufe schwaecher als eine klickbare — entweder als kunden-app-v2.html wieder einhaengen (noindex, Hinweis auf den historischen Stand) oder im Pitch als Screenshot zeigen." },

    { id: "n-mitarbeiterapp-hinter-kundenlogin", page: "kundenportal", selector: "main", status: "offen", done: true,
      title: "Mitarbeiter-App hinter der Kundenanmeldung — im Planspiel in Ordnung",
      text: "ENTSCHIEDEN 22.08.2026 durch Josh: bleibt so. Eine Pflegekraft meldet sich fachlich nicht ueber ein Kundenkonto an. Der Zugang liegt trotzdem dort, weil diese Maske im Planspiel der eine Einstieg in alle Prototypen ist — dieselbe Ueberlegung wie bei der Kunden-App (E-020). Abgemildert: die Auswahlgruppe heisst jetzt \"Apps · Prototypen\" statt \"Kunden-App\", die Kennung lautet s.koehler@lebenszeit-intern.beispiel, und der Hinweis sagt ausdruecklich \"Kein Kundenzugang\". Sauber waere eine eigene Mitarbeiteranmeldung, etwa neben der Cockpit-Anmeldung, die schon eine interne Rollenwahl hat. Vor dem Pitch entscheiden, ob das gebaut wird oder ob der Umweg als Planspiel-Vereinfachung stehenbleibt." },

    { id: "n-apps-eigenes-tokensystem", page: "allgemein", selector: "main", status: "spaeter", done: true,
      title: "Zweites Gestaltungssystem in den Prototypen — vorerst in Ordnung",
      text: "ENTSCHIEDEN 22.08.2026 durch Josh: erstmal in Ordnung, spaeter erneut ansehen. Beide Dateien bringen eigene Tokens und eine eigene Schriftfamilie mit: Atkinson Hyperlegible als Grundschrift, Literata fuer Serifen, IBM Plex Mono, dazu ein eigener Hell/Dunkel-Satz. Das Websystem verwendet Jost und Cormorant Garamond, und die Wortmarke gehoert laut CD in Georgia oder Times New Roman — hier steht sie in Literata. Unangetastet gelassen, weil derselbe Fall bei app-demo.html schon entschieden wurde: zwei Tokensysteme im selben Dokument ueberschreiben sich, sechs Namen sind doppelt belegt. Anders als app-demo liegen diese beiden aber nicht im Rahmen einer Hostseite, sondern sind selbst Seiten der Auslieferung — die Abweichung ist damit direkt sichtbar. Entscheidung noetig: als bewusste Plattformabweichung festhalten (wie bei app-demo) oder die Rail-Chrome auf das Websystem umstellen und nur den Telefoninhalt eigenstaendig lassen." },
    { id: "n-done-buehne-vollbild", page: "allgemein", selector: "main", status: "offen", done: true,
      title: "Geraetehuelle war keine Groessenangabe — Buehne und Vollbild ergaenzt",
      text: "BEHOBEN 22.08.2026. Die Huelle bezog ihre Hoehe aus min(854px, 100vh - 120px). Damit schrumpfte nicht die DARSTELLUNG, sondern das Geraet selbst: die Flexspalte im Inneren rechnete mit der verkleinerten Hoehe, die Reiterleiste rutschte hoch, und wie gross das Telefon eigentlich ist, war nicht ablesbar. Nachgemessen bei 1440x900 war die Huelle 679px hoch — ein Telefon dieser Groesse gibt es nicht. Jetzt steht die Huelle fest auf 393x852 und wird als Ganzes ueber transform:scale() skaliert; ein Platzhalterkasten in der skalierten Groesse haelt den Textfluss richtig, weil transform das Layout nicht veraendert. Das Innere rechnet dadurch immer mit den echten Geraetemassen. Die Huelle sitzt auf einer abgesetzten Buehne (Markengruen 9 Prozent ueber der Kartenflaeche, in beiden Modi geprueft) mit drei Chips darueber: Fiktiv-Hinweis, 393 x 852 pt, und der laufende Anzeigefaktor. Der Faktor ist der eigentliche Punkt — er sagt, dass man 75 Prozent eines echten Geraets sieht, statt es zu verschweigen. Ausserdem nie ueber 100 Prozent im Normalfall, sonst luegt die Massangabe. Vollbild ueber die Fullscreen-API auf der Buehne, Rueckfall auf eine fixierte Flaeche fuer Umgebungen ohne Element-Vollbild (iOS Safari), beide in derselben Skalenrechnung, dort bis Faktor 2,4. Nachgemessen 1440x900: 75 auf 97 Prozent, Telefon 823px. Nachtrag 22.08.2026, Anordnung nach Vorgabe von Josh: die Bedienknoepfe (Vollbild, Darstellung, Funkloch, Perspektivwechsel Hildegard/Markus) stehen jetzt in einer Leiste UEBER dem Telefon, die Angaben zu Aufloesung und Anzeigefaktor in einer Zeile DARUNTER. Trennung nach Funktion: oben greift man ein, unten liest man ab. In der Werkzeugzeile des Rails bleibt nur \"Zum Kundenportal\", weil das Navigation ist und keine App-Bedienung. Die Skalenrechnung zieht jetzt beide Zeilen ab, nicht nur eine — sonst waere das Telefon um die Hoehe der Fusszeile zu gross gerechnet und unten angeschnitten. Innenabstaende der Buehne dafuer von 18/24 auf 11/14 und die Luecke von 13 auf 9 verengt: der Anzeigefaktor liegt damit wieder bei 98 Prozent auf 1600x1100 und 75 auf 1440x900, also genau wie vor dem Umbau. ENTFERNT auf Wunsch von Josh: der Chip \"Fiktives Schulungsprojekt\". Damit gilt der Hinweis nur noch ueber die Laufleiste von lz-notes.js — und die rendert der Browser im Vollbild nicht mit, weil dort ausschliesslich die Buehne gezeigt wird. Der Vollbildmodus ist also ungekennzeichnet. Falls das stoert, waere eine im Vollbild eingeblendete Zeile der kleinste Weg zurueck. Nachtrag 23.08.2026: der Vollbildmodus umfasst jetzt nur noch das Telefonfeld (#phoneBuehne) statt der ganzen Buehne — Bedienleiste und Angabenzeile liegen ausserhalb und werden dort nicht mitgerendert, im Vollbild ist die App allein zu sehen. Die Skalenrechnung misst seither dieses Feld statt die Buehne minus zwei Zeilenhoehen; das ist nicht nur kuerzer, sondern auch robuster, weil eine spaeter ergaenzte Zeile nicht vergessen werden kann. Einziges Bedienelement im Vollbild bleibt ein schwach gesetzter Knopf oben rechts — ohne ihn waere der Rueckfallmodus eine Falle, weil dort kein Browser-Escape greift. Nachgemessen 1400x880: Anzeige 72 auf 101 Prozent, Vollbild-Element phoneBuehne, im Bildschirmfoto ist ausser dem Geraet nichts zu sehen." },

    { id: "n-done-vollbild-zeitschranke", page: "allgemein", selector: "main", status: "offen", done: true,
      title: "requestFullscreen konnte den Knopf tot wirken lassen",
      text: "BEHOBEN 22.08.2026. Nebenbefund beim Pruefen. requestFullscreen() liefert nicht ueberall ein Promise, und in manchen Umgebungen wird es weder erfuellt noch abgelehnt — in headless Chromium nachgewiesen, der Playwright-Aufruf lief in die Zeitgrenze. Ein .catch() faengt das nicht, weil nie etwas abgelehnt wird. Der Knopf haette dort nichts getan, ohne Fehlermeldung. Jetzt prueft eine Zeitschranke nach 350ms, ob der Vollbildmodus tatsaechlich greift, und schaltet sonst auf den Rueckfall um. Der Knopf tut damit immer etwas." },

    { id: "n-done-skala-schmal", page: "allgemein", selector: "main", status: "offen", done: true,
      title: "Telefon war auf dem Telefon am kleinsten",
      text: "BEHOBEN 22.08.2026. Die Skala nahm das Minimum aus Breiten- und Hoehenverhaeltnis. Auf einem 390px-Geraet liess die Breite 93 Prozent zu, die Hoehe aber nur 58 — herausgekommen sind 58 Prozent, also die kleinste Darstellung genau dort, wo der Prototyp am naechsten an seinem Zielgeraet ist. Unter 700px waechst die Buehne jetzt mit und die Seite scrollt; dort zaehlt nur die Breite. Nachgemessen 390x844: 87 statt 58 Prozent." },

    { id: "n-done-caps-2026", page: "allgemein", selector: "main", status: "offen", done: true,
      title: "Sachleistungsbetraege auf den Rechtsstand 2026 gezogen",
      text: "BEHOBEN 22.08.2026. Quelle ist die Mentorenreferenz auf Confluence-Seite 09, dort mit BMG-Beleg: PG 2 796 EUR, PG 3 1.497, PG 4 1.859, PG 5 2.299, Entlastungsbetrag 131. Die Mentoren-Kohortentabelle rechnet PG 2 ausdruecklich als 927 EUR, naemlich 796 plus 131. Angepasst: cockpit.html Tabelle DB.caps und alle 180 Klienten — cap, abgerufen und offen je Klient neu gerechnet, die Ausschoepfungsquote blieb dabei unangetastet, damit sich nur die Bezugsgroesse aendert und nicht die Aussage. kundenportal.html beide Konten (Stemmer 761/312 auf 796/326 bei gehaltenen 41 Prozent, Doerre auf 796/247 bei 31 Prozent). lz-quellen.js zwei Quellenhinweise samt Ø-Cap. praesentation.html das Graumarkt-Argument: es vergleicht die Caps gegen 1.500 bis 2.500 EUR Vollzeitbetreuung, und die Aussage haelt weiter — PG 3 liegt mit 1.497 unter der Untergrenze, PG 4 und 5 darueber. Der Abstand ist allerdings von 68 auf 3 EUR geschrumpft; das Argument steht jetzt auf drei Euro und sollte im Pitch nicht als Rechnung, sondern als Groessenordnung vorgetragen werden. kunden-app.html Entlastungsbetrag 125 auf 131 an zwei Stellen. NICHT angefasst: prompt-tagebuch.html nennt 125 EUR als Wortprotokoll einer Sprint-1-Antwort und deckt sich mit dem Personas-Handout — ein Protokoll wird nicht rueckdatiert. Ebenso nicht die Registry-Texte hier, die die alten Werte absichtlich zitieren. app-demo.html war ein Fehlalarm der Suche: die 2200 dort sind Millisekunden in einem setTimeout." },

    { id: "n-cap-mittelwert-weicht-ab", page: "allgemein", selector: "main", status: "offen",
      title: "Ø-Cap im Kanon passt nicht zu unseren 180 Klienten",
      text: "GEFUNDEN 22.08.2026 beim Nachrechnen der neuen Caps. Unser Klientenbestand verteilt sich auf 71 Faelle in PG 2, 64 in PG 3, 35 in PG 4 und 10 in PG 5. Mit den Betraegen 796/1.497/1.859/2.299 ergibt das einen Ø-Cap von 1.335,44 EUR, einen Jahresanspruch von 2.885 kEUR und 28.845 EUR je Prozentpunkt Ausschoepfung. Der Kanon nennt 1.339,33 · 2.893 · 28.930. Die Abweichung ist klein, aber sie bedeutet, dass die drei abgeleiteten Werte im Kanon aus einer anderen Pflegegradverteilung stammen als aus dem Bestand, mit dem Cockpit und Portal rechnen. Vor dem Pitch klaeren, welche Verteilung gilt — sonst stehen zwei Ausschoepfungsrechnungen mit verschiedenen Nennern nebeneinander, und genau das ist die Fehlerklasse, gegen die der Kanon gebaut wurde." },

    { id: "n-done-klienten-aus-bestand", page: "allgemein", selector: "main", status: "offen", done: true,
      title: "Restliche Tour-Klientinnen aus dem Cockpit-Bestand besetzt",
      text: "BEHOBEN 22.08.2026 auf Anweisung von Josh. Die drei verbliebenen Namen der Tour standen nicht im 180er-Bestand und sind jetzt daraus besetzt, jeweils mit passender Gemeinde und passendem Pflegegrad: Heinrich Weber (Anroechte) auf Friedrich Aldegarmann, Am Haarstrang 13 · Elfriede Koch (Bad Sassendorf, PG 3) auf Frieda Schaefermeyer, Kurparkallee 60 · Wilhelm Bathe (Lippetal, PG 4) auf Margarete Ruether, Auenstrasse 11. Damit stammen alle sechs Tourstopps aus demselben Bestand wie Cockpit und Portal. Eine Aenderung war dabei mehr als ein Namenstausch: der erste Stopp stand auf Pflegegrad 1, und unser Bestand hat keinen einzigen PG-1-Fall — die 180 sind laut Kanon und E-029 PG 2 bis 5. Der Stopp steht jetzt auf PG 2. Wer die 27 PG-1-Faelle der Mentorenkohorte in der App zeigen will, braucht dafuer erst einen eigenen Bestand; sie lassen sich nicht aus den 180 entnehmen." },
    { id: "n-done-markus-ueberschrift", page: "kunden-app", selector: "main", status: "offen", done: true,
      title: "Markus' Abendbildschirm hatte keine Ueberschrift, nur eine Unterzeile",
      text: "BEHOBEN 23.08.2026, von Josh gemeldet. Zwoelf der dreizehn Ansichten bauen ihren Kopf ueber die gemeinsame Funktion head(eyebrow, titel, sub) und bekommen den Titel damit als h2.h-view — 29px, fett. Genau eine Ansicht baute ihn von Hand: heuteM(), der Sonntagabend-Bildschirm von Markus. Dort stand \"Guten Abend, Markus\" als p.sub, also in der Klasse fuer UNTERzeilen: 15px, graue Sekundaerfarbe. Der Bildschirm sah dadurch aus, als fehle ihm der Titel — und weil direkt darunter die 31px grosse Aussage \"Ihrer Mutter geht es gut\" steht, wirkte die Begruessung wie eine verrutschte Bildunterschrift. Jetzt ueber head() wie ueberall, nachgemessen 29px/700. Der Punkt ist nicht die eine Zeile, sondern dass sie von Hand gebaut war: eine Ansicht, die die gemeinsame Funktion umgeht, faellt bei jeder kuenftigen Aenderung an head() wieder aus der Reihe. Im Bestand ist das jetzt die einzige gewesene Stelle — geprueft, es gibt keinen zweiten handgebauten Kopfblock." },
    { id: "n-sachleistung-2024er-werte", page: "allgemein", selector: "main", status: "offen", done: true,
      title: "Sachleistungsbeträge standen auf dem Stand von 2024 — erledigt",
      text: "GEFUNDEN 20.08.2026 beim Confluence-Abgleich. Nachtrag 22.08.: der Fehler sitzt in cockpit.html als Tabelle DB.caps = {2:761, 3:1432, 4:1778, 5:2200} — alle vier Stufen sind der Stand 2024. Kanon nennt 796 / 1.497 / 1.859 / 2.299. Cockpit, Kundenportal und Kunden-App rechnen mit 761 EUR Sachleistungsanspruch fuer Pflegegrad 2. Das ist der Betrag von 2024. Seit dem 01.01.2025 sind es 796 EUR, und dabei bleibt es 2026 — gegen zwei unabhaengige Quellen geprueft. Dieselbe Fehlerklasse steckt im PG-3-Wert 1.432 EUR, ebenfalls 2024; der geltende Betrag ist noch zu belegen. Wichtig zur Abgrenzung: das ist NICHT die Pflegegrad-Verwechslung von Hildegard Stemmer (n-persona-alter, behoben 13.08.) — der Pflegegrad stimmt jetzt, nur die dahinterliegende Betragstabelle ist veraltet. Die Zahl ist deshalb heikel, weil daraus der Eigenanteil folgt und der laut Designgrundlage der emotional wichtigste Wert der Zielgruppe ist: ein zu niedriger Anspruch erzeugt einen zu hohen Eigenanteil. Vor der Korrektur klaeren, welches Jahr das Planspiel abbildet — bei durchgehend 2026 muessen alle Betragsstufen mit, nicht nur PG 2. Im Kanon als offener Zahlenanker eingetragen. BEHOBEN 22.08.2026 mit der Mentorenreferenz, siehe n-done-caps-2026." }
  ];
